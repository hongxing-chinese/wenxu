...
import { generateDivinationSession, type DivinationDraft } from '@/lib/divination/engine';
import {
...
import { addDivinationHistory } from '@/lib/history-records';
...
export function getSummaryBlocks(method: DivinationDraft['method'], data: DivinationData) {
  switch (method) {
    case 'liuyao':
      return {
        title: '六爻起卦结果',
...
export function DivinationPanel() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // 跳转到结果页，传递记录 ID
      navigate(`/result?mode=divination&record=${savedRecord.id}`);
    } catch (currentError) {
