...
import { hasLocalLLMConfig } from '@/lib/local-llm-config';
import { getDivinationHistoryById, type DivinationHistoryRecord } from '@/lib/history-records';
...
function resolveBaziShortcutMode(
  ...
  if (analysisMode === 'compatibility') {
    return (
      ...
      getBaziShortcutActions(analysisMode)[0]?.label ?? '综合'
    );
  }
  ...
  return matched?.label ?? getBaziShortcutActions(analysisMode)[0]?.label ?? '综合';
}
...
function resolveZiweiShortcutMode(
  ...
  if (analysisMode === 'compatibility') {
    return (
      ...
      getZiweiShortcutActions(analysisMode)[0]?.label ?? '综合'
    );
  }
  ...
  return matched?.label ?? getZiweiShortcutActions(analysisMode)[0]?.label ?? '综合';
}
...
function ZiweiScopeModal(props: {
  ...
  const initialFallbackDateRef = useRef(fallbackScopeDateStr);
  ...
  useEffect(() => {
    ...
    setDraftYearDateStr(initialFallbackDateRef.current);
    setDraftMonthDateStr(buildZiweiMonthAnchorDate(initialFallbackDateRef.current));
    setDraftDayDateStr(initialFallbackDateRef.current);
  }, [initialDecadalIndex, normalizedSelectedScope]);
...
export function ResultPage() {
  ...
  const [isLocalLLMConfigOpen, setIsLocalLLMConfigOpen] = useState(false);
  const [pendingAutoSend, setPendingAutoSend] = useState('');
  const [localLLMConfigVersion, setLocalLLMConfigVersion] = useState(0);
  const chatMode: ChatMode = hasLocalLLMConfig() ? 'local' : 'cloud';
  const isDivinationMode = searchParams.get('mode') === 'divination';
  const divinationRecordId = searchParams.get('record');
  const [divinationRecord, setDivinationRecord] = useState<DivinationHistoryRecord | null>(null);
  const [divinationChatPrompt, setDivinationChatPrompt] = useState<string | null>(null);
  ...
  // 占卜模式：从 URL 参数加载记录
  useEffect(() => {
    if (!isDivinationMode || !divinationRecordId) return;
    const record = getDivinationHistoryById(divinationRecordId);
    if (!record) {
      setDivinationRecord(null);
      return;
    }
    setDivinationRecord(record);
    setDivinationChatPrompt(record.session.prompt);
  }, [isDivinationMode, divinationRecordId]);

  // 占卜模式：显示占卜结果和 ChatPanel 的垂直堆叠布局
  if (isDivinationMode) {
    if (!divinationRecord) {
      // 记录未找到或正在加载
      return (
        <div className="page-shell">
          <PageTopbar
            title="AI 解读"
            onBack={() => navigate('/app?mode=divination')}
            rightAction={
              <button type="button" className="auth-config-btn" onClick={() => setIsLocalLLMConfigOpen(true)} title="本地大模型配置">⚙</button>
            }
          />
          <div className="result-tab-stage" style={{ padding: '1rem' }}>
            <div className="divination-loading">加载中...</div>
          </div>
          {isLocalLLMConfigOpen ? (
            <LocalLLMConfigModal
              onClose={() => setIsLocalLLMConfigOpen(false)}
              onSaved={() => setLocalLLMConfigVersion((v) => v + 1)}
            />
          ) : null}
        </div>
      );
    ...
    return (
      <div className="page-shell">
        ...
        />
        <div className="divination-result-chat-stack">
          {/* 上方：占卜结果 */}
          <DivinationResult draft={divinationRecord.draft} session={divinationRecord.session} />

          {/* 下方：ChatPanel，自动发送提示词 */}
          <div className="divination-chat-panel-wrapper">
            <ChatPanel
              systemPrompt="你是一个精通中国传统命理文化的 AI 助手，擅长解读占卜结果（六爻、梅花易数、奇门遁甲、大六壬、塔罗等）。请根据用户提供的占卜信息，给出专业、详尽且易于理解的解读。"
              defaultQuestion=""
              mode={chatMode}
              onConfigRequest={() => setIsLocalLLMConfigOpen(true)}
              initialPrompt={divinationChatPrompt ?? undefined}
              initialDisplay={divinationRecord.session.question}
            />
          </div>
        </div>
        {isLocalLLMConfigOpen ? (
  ...
  return (
    <div className="page-shell">
      <PageTopbar
        ...
        wide
        onBack={() =>
          navigate(`/app?mode=${inputState.analysisMode === 'compatibility' ? 'compatibility' : 'single'}`)
        }
        rightAction={
            <button type="button" className="auth-config-btn" onClick={() => setIsLocalLLMConfigOpen(true)} title="本地大模型配置">⚙</button>
          }
      ...
      <div className="result-tab-stage">
            ...
            <div className="single-panel-shell">
              <section className="panel result-panel result-panel-ziwei">
                ...
                {inputState.analysisMode === 'compatibility' && currentZiweiPayload && partnerZiweiPayload ? (
                  <div className="result-stack-layout">
        ...
        >
          {mountedTabs.prompt ? (
          <div className="prompt-chat-stack">
            ...
            <ChatPanel
              systemPrompt={latestActivePromptText || ''}
              defaultQuestion={promptState.promptSource === 'bazi' ? finalBaziQuestion : effectiveZiweiQuickQuestion}
              mode={chatMode}
              onConfigRequest={() => setIsLocalLLMConfigOpen(true)}
              autoSend={
                promptState.promptSource === 'bazi'
              ...
              pendingSend={pendingAutoSend || undefined}
            />
          </div>
          ) : null}
        </div>
